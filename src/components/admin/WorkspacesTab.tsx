    ...
    <div className="space-y-4">
            ...
            <>
              ...
              <div className="mt-6 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button
                      variant={selectedWorkspace.subscription_status === 'canceled' ? 'default' : 'destructive'}
                      size="sm"
                      onClick={() =>
                        setConfirmAction({
                          type: selectedWorkspace.subscription_status === 'canceled' ? 'unblock' : 'block',
                          workspace: selectedWorkspace,
                        })
                      }
                    >
                      {selectedWorkspace.subscription_status === 'canceled' ? (
                        <>
                          <Shield className="mr-2 h-4 w-4" />
                          Reativar
                        </>
      ...
      <AlertDialog open={!!confirmAction} onOpenChange={() => setConfirmAction(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {confirmAction?.type === 'block' && 'Bloquear Workspace'}
              {confirmAction?.type === 'unblock' && 'Reativar Workspace'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {confirmAction?.type === 'block' &&
                `Tem a certeza que quer bloquear "${confirmAction.workspace.name}"? Os utilizadores perderão o acesso.`}
              {confirmAction?.type === 'unblock' &&
                `Tem a certeza que quer reativar "${confirmAction.workspace.name}"?`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

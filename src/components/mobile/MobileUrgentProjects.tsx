...
interface MobileUrgentProjectsProps {
...
export function MobileUrgentProjects({ 
  urgentProjects, 
  loading, 
  onProjectClick,
  maxItems = 3,
}: MobileUrgentProjectsProps) {
  const navigate = useNavigate();
  const displayedProjects = urgentProjects.slice(0, maxItems);
  const hasMore = urgentProjects.length > maxItems;
      ...
      <Card className="glass-card">
        <CardHeader className="flex flex-row items-center justify-between py-3 px-4">
          ...
          {hasMore && (
            ...
            >
              +{urgentProjects.length - maxItems}
            ...
            <div className="space-y-2">
              {displayedProjects.map((project) => {
                ...
                return (
                  <button
                    key={project.id}
                    className="w-full flex items-center justify-between p-3 rounded-xl bg-muted/30 active:bg-muted/50 transition-all border border-transparent active:border-primary/20"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <Badge 
                        variant="outline" 
                        className={cn('text-[10px] capitalize px-1.5 py-0.5 h-5', getPriorityColor(project.priority))}
                      >
                        {project.priority}
                      </Badge>
                    </div>

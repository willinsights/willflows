...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'avg_per_week', ROUND(COUNT(*)::NUMERIC / GREATEST(1, EXTRACT(WEEK FROM (p_end_date - p_start_date))), 1)
  ) INTO v_throughput
  FROM projects
  ...
  SELECT jsonb_build_object(
    ...
    'median_days', ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (delivered_at - created_at)) / 86400)::NUMERIC, 1)
  ) INTO v_cycle_time
  FROM projects
  WHERE workspace_id = p_workspace_id
    AND is_delivered = true

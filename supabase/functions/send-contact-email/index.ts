...
serve(async (req: Request) => {
  ...
  try {
    ...
    const emailResponse = await resend.emails.send({
      ...
      subject: `[Contacto WillFlow] ${escapeHtml(sanitizedSubject)}`,
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            <tr>
              ...
              <td style="padding: 8px 12px;">${escapeHtml(sanitizedName)}</td>
            </tr>
            <tr>
              <td style="padding: 8px 12px; font-weight: bold; color: #555;">Email:</td>
              <td style="padding: 8px 12px;"><a href="mailto:${escapeHtml(sanitizedEmail)}">${escapeHtml(sanitizedEmail)}</a></td>
            </tr>
            <tr>
              <td style="padding: 8px 12px; font-weight: bold; color: #555;">Assunto:</td>
              <td style="padding: 8px 12px;">${escapeHtml(sanitizedSubject)}</td>
            </tr>
          </table>
          <div style="background: #f9fafb; border-radius: 8px; padding: 16px; margin-top: 16px;">
            ...
            <p style="white-space: pre-wrap; color: #333; line-height: 1.6; margin: 0;">${escapeHtml(sanitizedMessage)}</p>
          </div>
